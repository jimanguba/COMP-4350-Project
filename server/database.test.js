...
test('getBook runs the correct SELECT statement with the correct parameter', () => {
    ...
    expect(pool.query).toBeCalledTimes(1)
    expect(pool.query).toBeCalledWith('SELECT * FROM books WHERE book_id = $1', [1])
...
test('Insert book with a valid book to return the result of the query', () => {
    ...
    expect(pool.query).toBeCalledTimes(1)
    expect(pool.query).toBeCalledWith(insertQuery, ['The Eye of the World', 'Robert Jordan', 832, 'High Fantasy', 'The Eye of the World', 'Robert Jordan'])
