...
export default function CompletedBookButton({book_id}) {
    ...
    return (
        <button className={`readingStateButton ${onCompletedBooks ? "onCompletedBooks" : "offCompletedBooks"}`} onClick={toggleReadingState}>{onCompletedBooks ? "Completed" : "Mark as Completed"}</button>
    )
}
