...
const ReviewCard = ({ review, addReply }) => {
  ...
  useEffect(() => {
    ...
    if (reviewReplies.length > 0) {
      ...
      const uniqueUserIds = [...new Set(reviewReplies.map((reply) => reply.user_id))];
  ...
  {reviewReplies.map((reply, index) => (
    <div key={index} className="review-reply">
      <div className="review-reply-header">
        <span className="reply-user-name">{replyUserNames[reply.user_id] || 'Anonymous'}</span>
        <span className="reply-date">{new Date(reply.reply_date).toLocaleDateString()}</span>
      </div>
