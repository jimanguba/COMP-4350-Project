...
describe('StatisticsYear component', () => {
    ...
    it('renders correctly with average reading time and rating', async () => {
        ...
        await waitFor(() => {
            expect(getByText('Your average reading time is 60 minutes')).toBeInTheDocument();
    ...
    it('logs errors to console if fetching data fails', async () => {
        ...
        await waitFor(() => {
            expect(console.error).toHaveBeenCalledWith('Error fetching average reading time:', new Error('Failed to fetch average time'));
